...
<div id="inhoadon">
    <?php 
    $select1 = "select * from hoadon where mahd={$_GET['mahd']}";
    $query1 = mysqli_query($link, $select1);
    $dem1 = mysqli_num_rows($query1);
$bien = mysqli_fetch_array($query1);
 $abc = $_GET['mahd']; 
    if ($bien['trangthai']==2) echo 
    "<p style='float:right; margin-top:10px; padding-right:30px;'><a href='inhd.php?mahd=$abc' target='_blank'>In hoá đơn</a> </p>";
    else echo "";?>
</div>